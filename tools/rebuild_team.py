...
<style>
    ...
    .team-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 16px; position: relative; z-index: 1; }
    .team-header h2 { color: #fff; margin: 0; font-size: 1.4rem; }
    .team-header p { color: rgba(255,255,255,.55); margin: 4px 0 0; font-size: 0.85rem; }
    .team-stats { display: flex; gap: 28px; text-align: center; }
    .team-stat .ts-val { font-size: 1.6rem; font-weight: 700; color: #fff; line-height: 1; }
    .team-stat .ts-lbl { font-size: 0.65rem; text-transform: uppercase; color: rgba(255,255,255,.4); letter-spacing: 1.2px; margin-top: 2px; }
    ...
    .roster-pod.available .roster-sprite img { filter: grayscale(.8) brightness(.4); }
    .roster-pod.available .roster-name { color: rgba(255,255,255,.35); }
    .roster-pod.available .roster-role { color: rgba(255,255,255,.2); }
    .roster-pod.available:hover .roster-sprite img { filter: grayscale(.3) brightness(.7); }
    .roster-pod.available:hover .roster-name { color: rgba(255,255,255,.7); }

    .roster-pod.hired {
        background: rgba(255,255,255,.07);
        border-color: rgba(255,255,255,.12);
    }
    .roster-pod.hired:hover {
    ...
    .roster-pod.working {
        ...
        box-shadow: 0 0 24px rgba(99,102,241,.15), inset 0 0 24px rgba(99,102,241,.05);
        animation: podGlow 2.5s ease-in-out infinite alternate;
    }
    @keyframes podGlow {
        0% { box-shadow: 0 0 20px rgba(99,102,241,.1), inset 0 0 20px rgba(99,102,241,.03); }
        100% { box-shadow: 0 0 32px rgba(99,102,241,.25), inset 0 0 32px rgba(99,102,241,.06); }
    }
    ...
    .roster-pod:hover .roster-sprite { transform: scale(1.08); }
    .roster-sprite img {
    ...
    .roster-dot {
        ...
        border: 2px solid #1a1a2e; z-index: 2;
    }
    .dot-idle { background: #10b981; }
    .dot-working { background: #6366f1; animation: dotPulse 1.2s infinite; }
    ...
    .roster-hire-tag {
        ...
        letter-spacing: 1px; padding: 3px 14px; border-radius: 10px;
    ...
    /* Status text for hired agents */
    .roster-status-text {
        font-size: .6rem; font-weight: 600; margin-top: 6px;
        padding: 2px 10px; border-radius: 8px; display: inline-block;
        letter-spacing: .3px; text-transform: uppercase;
    }
    .rst-working { background: rgba(99,102,241,.2); color: #a5b4fc; }
    .rst-idle { background: rgba(16,185,129,.15); color: #6ee7b7; }
    .rst-standby { background: rgba(255,255,255,.06); color: rgba(255,255,255,.35); }

    /* Finding badge on roster pod */
    .roster-finding-badge {
        position: absolute; top: 8px; right: 8px;
        min-width: 20px; height: 20px; border-radius: 10px;
        display: flex; align-items: center; justify-content: center;
        font-size: .6rem; font-weight: 700; color: #fff; padding: 0 5px;
        z-index: 3;
    }
    .rfb-critical { background: #ef4444; }
    .rfb-warning { background: #f59e0b; }
    .rfb-positive { background: #10b981; }
    ...
    .agent-card {
        ...
        padding: 24px; transition: box-shadow 0.2s, border-color 0.2s; position: relative;
    }
    ...
    @media (max-width: 1100px) {
        .roster-grid { grid-template-columns: repeat(5, 1fr); gap: 10px; }
        .roster-sprite { width: 72px; height: 72px; }
        .roster-name { font-size: .75rem; }
    }
    @media (max-width: 768px) {
        .roster-grid { grid-template-columns: repeat(3, 1fr); gap: 10px; }
        .roster-sprite { width: 64px; height: 64px; }
    ...
    @media (max-width: 480px) {
        ...
        .roster-pod { padding: 14px 8px 12px; }
        .roster-sprite { width: 56px; height: 56px; }
    }
</style>
...
<div class="team-wrap">
    <div class="team-header">
        <div>
            ...
            <p>Your AI marketing team. Hire, train, put them to work.</p>
        </div>
        <div class="team-stats">
            <div class="team-stat">
                <div class="ts-val" id="stat-hired">0</div>
                <div class="ts-lbl">Hired</div>
            </div>
            <div class="team-stat">
                <div class="ts-val" id="stat-active">0</div>
                <div class="ts-lbl">Working</div>
            </div>
            <div class="team-stat">
                <div class="ts-val" id="stat-findings">-</div>
                <div class="ts-lbl">Findings</div>
            </div>
        </div>
    </div>
    ...
    <div class="team-hero-actions">
        ...
        <span id="run-status" style="margin-left:12px;font-size:0.85rem;opacity:0.85;color:#fff;"></span>
    </div>
    ...
    <div class="hire-modal">
        ...
        <div class="hire-modal-body">
            ...
            <div class="hire-step" id="hire-step-2">
                ...
                <div class="train-hint">You can always update this later in Settings > Team Instructions.</div>
            ...
            <div class="hire-step" id="hire-step-3">
                <div style="text-align:center;padding:20px 0;">
                    ...
                    <p style="font-size:.88rem;color:#6b7280;" id="hire-done-msg">They're at their desk and ready to work.</p>
...
// ── ROSTER ──
function renderRoster(latestData) {
    ...
    const html = AGENTS.map(agent => {
        ...
        const agentLatest = latest[agent.key];
        const isWorking = agentLatest && agentLatest.status === 'in_progress';
        const color = AGENT_COLORS[agent.key] || '#6b7280';

        let stateClass = 'available';
        if (isHired && isWorking) stateClass = 'hired working';
        else if (isHired) stateClass = 'hired';

        let statusHtml = '';
        if (!isHired) {
            statusHtml = '<div class="roster-hire-tag">HIRE</div>';
        } else if (isWorking) {
            statusHtml = '<span class="roster-status-text rst-working">Working</span>';
        } else if (agentLatest) {
            statusHtml = '<span class="roster-status-text rst-idle">Ready</span>';
        } else {
            statusHtml = '<span class="roster-status-text rst-standby">Standby</span>';
        }

        return `
        <div class="roster-pod ${stateClass}" data-agent="${agent.key}" onclick="openHireModal('${agent.key}')">
            <div class="roster-sprite">
                <img src="/static/sprites/agents/${agent.key}.png" alt="${agent.name}">
                ${isHired ? `<span class="roster-dot ${isWorking ? 'dot-working' : 'dot-idle'}"></span>` : ''}
            </div>
            <div class="roster-name">${agent.name}</div>
    ...
    const hiredCount = Object.keys(hiredAgents).length;
    document.getElementById('stat-hired').textContent = hiredCount + '/' + AGENTS.length;
}
...
function openHireModal(agentKey) {
    ...
    document.getElementById('hire-modal-skills').innerHTML =
        agent.skills.map(s => `<span>${s}</span>`).join('');

    document.querySelectorAll('.hire-step').forEach(s => s.classList.remove('active'));
    ...
    if (!isHired) {
        ...
        document.getElementById('hire-done-msg').textContent = 'Trained and ready. They are actively working on your account.';
    }
...
function populateTrainStep(agentKey, agentName) {
    const g = TRAIN_GUIDANCE[agentKey];
    const guidanceEl = document.getElementById('train-guidance');
    const questionsEl = document.getElementById('train-questions');
    ...
    if (g) {
        ...
        questionsEl.innerHTML = '<div class="train-q-list">' +
            g.questions.map((q, i) => `<div class="train-q-item"><span class="train-q-num">${i+1}</span>${q}</div>`).join('') +
            '</div>';
    } else {
        guidanceEl.textContent = 'Tell ' + agentName + ' about your business so they can do better work.';
...
function confirmHire() {
    ...
    const btn = document.getElementById('hire-btn-confirm');
    btn.disabled = true;
    ...
    .then(data => {
        if (data.success) {
            ...
            document.getElementById('hire-step-2').classList.add('active');
            document.getElementById('train-textarea').value = '';
            const hiredAgent = AGENTS.find(a => a.key === currentHireAgent);
...
function confirmTrain() {
    if (!currentHireAgent) return;
    const btn = document.getElementById('train-btn-confirm');
    const notes = document.getElementById('train-textarea').value.trim();
    btn.disabled = true;
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            setTimeout(() => {
                ...
                document.querySelectorAll('.hire-step').forEach(s => s.classList.remove('active'));
                document.getElementById('hire-step-3').classList.add('active');
                const agent = AGENTS.find(a => a.key === currentHireAgent);
...
function renderAgentCard(agent, findingCounts) {
    ...
    const isHired = !!hiredAgents[agent.key];
    const isTrained = isHired && hiredAgents[agent.key].trained;
    ...
    const currentText = !isHired
        ? 'Not hired yet - click to recruit'
    ...
    const cardClass = isWorking ? 'working' : (!isHired ? 'not-hired' : '');

    return `
    <div class="agent-card ${cardClass}" data-agent="${agent.key}" onclick="openHireModal('${agent.key}')" style="cursor:pointer;">
        ${badgeHtml}
...
function parseFindingExtra(f) {
    ...
    try { return JSON.parse(f.extra_json); } catch { return {}; }
}
...
function renderFinding(f) {
    ...
    const extra = parseFindingExtra(f);
    const steps = Array.isArray(extra.steps) ? extra.steps : [];
    let stepsHtml = '';
    if (steps.length) {
        stepsHtml = '<ol class="finding-steps">' + steps.map(s => `<li>${escapeHtml(s)}</li>`).join('') + '</ol>';
    }
    return `
    <div class="finding-card sev-${f.severity}" data-severity="${f.severity}" data-agent="${f.agent_key}" data-id="${f.id}">
        ...
        <button class="finding-assign-btn" onclick="event.stopPropagation();createTaskFromFinding(${f.id})" title="Create Task">
            <i class="bi bi-clipboard-plus"></i> Task
        </button>
        <button class="finding-dismiss" onclick="event.stopPropagation();dismissFinding(${f.id}, this)" title="Dismiss">
...
function renderFindings(findings) {
    ...
    if (!filtered.length) {
        const msg = findings.length > 0 ? `No ${currentFilter} findings.` : 'No findings yet. Hire agents and hit "Run My Team" to start.';
...
function dismissFinding(id, btn) {
    ...
    .then(r => r.json())
    .then(() => { card.remove(); allFindings = allFindings.filter(f => f.id !== id); updateFindingsStat(); })
...
function createTaskFromFinding(findingId) {
    ...
    var body = { finding_id: findingId, title: title, steps: steps };
    ...
    .then(function(data) {
        if (data.success) {
            alert('Task created! Go to Tasks to assign it to a team member.');
        } else {
...
function loadFindings() {
    fetch('{{ url_for("client.client_team_findings") }}')
        ...
        .then(data => {
            ...
            // Update finding badges on roster pods
            const rosterPods = document.querySelectorAll('.roster-pod');
            rosterPods.forEach(pod => {
                const key = pod.dataset.agent;
                const c = counts[key] || { critical: 0, warning: 0, total: 0 };
                let badge = pod.querySelector('.roster-finding-badge');
                if (c.critical > 0) {
                    if (!badge) { badge = document.createElement('span'); badge.className = 'roster-finding-badge'; pod.prepend(badge); }
                    badge.textContent = c.critical; badge.className = 'roster-finding-badge rfb-critical'; badge.style.display = '';
                } else if (c.warning > 0) {
                    if (!badge) { badge = document.createElement('span'); badge.className = 'roster-finding-badge'; pod.prepend(badge); }
                    badge.textContent = c.warning; badge.className = 'roster-finding-badge rfb-warning'; badge.style.display = '';
                } else if (c.total > 0) {
                    if (!badge) { badge = document.createElement('span'); badge.className = 'roster-finding-badge'; pod.prepend(badge); }
                    badge.textContent = c.total; badge.className = 'roster-finding-badge rfb-positive'; badge.style.display = '';
                } else if (badge) { badge.style.display = 'none'; }
            });
            // Update agent detail card badges
            const grid = document.getElementById('agent-grid');
            if (grid.children.length > 0) {
                for (const agent of AGENTS) {
                    const card = grid.querySelector(`[data-agent="${agent.key}"]`);
                    if (!card) continue;
                    let cbadge = card.querySelector('.finding-count-badge');
                    const c = counts[agent.key] || { critical: 0, warning: 0, total: 0 };
                    if (c.critical > 0) {
                        if (!cbadge) { cbadge = document.createElement('span'); cbadge.className = 'finding-count-badge'; card.prepend(cbadge); }
                        cbadge.textContent = c.critical; cbadge.className = 'finding-count-badge'; cbadge.style.display = '';
                    } else if (c.warning > 0) {
                        if (!cbadge) { cbadge = document.createElement('span'); cbadge.className = 'finding-count-badge'; card.prepend(cbadge); }
                        cbadge.textContent = c.warning; cbadge.className = 'finding-count-badge sev-warning'; cbadge.style.display = '';
                    } else if (c.total > 0) {
                        if (!cbadge) { cbadge = document.createElement('span'); cbadge.className = 'finding-count-badge'; card.prepend(cbadge); }
                        cbadge.textContent = c.total; cbadge.className = 'finding-count-badge sev-positive'; cbadge.style.display = '';
                    } else if (cbadge) { cbadge.style.display = 'none'; }
                }
            }
        })
...
function runTeam() {
    ...
    status.textContent = 'Starting your team run...';
...
function pollRunStatus() {
    ...
    const interval = setInterval(() => {
        pollCount++;
        const phase = phases[Math.min(Math.floor(pollCount / 8), phases.length - 1)];
        const dots = '.'.repeat((pollCount % 3) + 1);
        status.textContent = phase.replace('...', dots);
...
function renderActivity(items) {
    if (!items || items.length === 0) {
        return '<div class="empty-feed"><i class="bi bi-check-circle"></i>No activity yet. Your team will start logging work once you run them.</div>';
    }
...
function loadTeamData() {
    ...
    fetch('{{ url_for("client.client_team_data") }}')
        ...
        .then(data => {
            ...
            renderRoster(latestMap);

            document.getElementById('agent-grid').innerHTML = agents.map(a => renderAgentCard(a, findingCounts)).join('');
        ...
        .catch(err => {
            ...
            renderRoster();
            document.getElementById('agent-grid').innerHTML = AGENTS.map(a => renderAgentCard(a, {})).join('');
