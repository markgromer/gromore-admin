...
_BOOTSTRAP_DB = str(_TEST_ROOT / "gromore-blog-editor-bootstrap.db")
...
class BlogEditorAssetTests(unittest.TestCase):
    def setUp(self):
        self.db_file = _TEST_ROOT / f"blog-editor-{uuid.uuid4().hex}.db"
        ...
        with self.app.app_context():
            self.brand_id = self.app.db.create_brand({
                "slug": f"blog-editor-{uuid.uuid4().hex[:8]}",
                "display_name": "Blog Editor Test Brand",
            })
            self.app.db.update_brand_text_field(self.brand_id, "wp_site_url", "https://example.com")
        ...
        with self.client.session_transaction() as session:
            ...
            session["client_brand_name"] = "Blog Editor Test Brand"
    ...
    def test_blog_editor_renders_asset_bootstrap(self):
        response = self.client.get("/client/blog/new")
