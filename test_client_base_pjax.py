...
class ClientBasePjaxTests(unittest.TestCase):
    ...
    def test_pjax_uses_exported_loader_message_helper(self):
        ...
        self.assertIn("Loading a new action plan", html)
        self.assertIn("Action Plan", html)
        self.assertNotIn("Mission Control", html)
        self.assertIn('nav-link-label">Pipeline</span>', html)
        self.assertIn('nav-link-label">Action Plan</span>', html)
        self.assertNotIn('nav-link-label">Missions</span>', html)
