...
_HEATMAP_ASYNC_POINT_THRESHOLD = 9
...
def client_save_facebook_page_id():
    ...
    if "instagram_account_id" in request.form:
        instagram_raw = (request.form.get("instagram_account_id") or "").strip()
        db.update_brand_api_field(brand_id, "instagram_account_id", instagram_raw)
    if "linkedin_organization_id" in request.form:
        linkedin_raw = (request.form.get("linkedin_organization_id") or "").strip()
        db.update_brand_api_field(brand_id, "linkedin_organization_id", linkedin_raw)
    if "nextdoor_business_url" in request.form:
        nextdoor_raw = (request.form.get("nextdoor_business_url") or "").strip()
        db.update_brand_api_field(brand_id, "nextdoor_business_url", nextdoor_raw)
    flash("Meta and social channel references saved.", "success")
    return_to = request.form.get("return_to")
    return redirect(url_for("client.client_connections") if return_to == "connections" else url_for("client.client_settings"))
...
def client_reviews_settings():
    ...
    widget_settings = {
        ...
        "background": request.form.get("reputation_widget_background") or "#ffffff",
        "header_background": request.form.get("reputation_widget_header_background") or "#12322b",
        "text_color": request.form.get("reputation_widget_text_color") or "#0f172a",
        "muted_color": request.form.get("reputation_widget_muted_color") or "#64748b",
        "star_color": request.form.get("reputation_widget_star_color") or "#f59e0b",
        "border_color": request.form.get("reputation_widget_border_color") or "#dbe5df",
        "button_text_color": request.form.get("reputation_widget_button_text_color") or "#ffffff",
        "avatar_color": request.form.get("reputation_widget_avatar_color") or "#2563eb",
        "width": request.form.get("reputation_widget_width") or "520",
        "radius": request.form.get("reputation_widget_radius") or "12",
        "shadow": request.form.get("reputation_widget_shadow") or "medium",
        "font": request.form.get("reputation_widget_font") or "system",
...
(function() {{
  ...
  function color(key, fallback) {{
    return /^#[0-9a-f]{{6}}$/i.test(settings[key] || '') ? settings[key] : fallback;
  }}
  var accent = color('accent', '#2563eb');
  var background = color('background', '#ffffff');
  var headerBackground = color('header_background', '#12322b');
  var textColor = color('text_color', '#0f172a');
  var mutedColor = color('muted_color', '#64748b');
  var starColor = color('star_color', '#f59e0b');
  var borderColor = color('border_color', '#dbe5df');
  var buttonText = color('button_text_color', '#ffffff');
  var avatarColor = color('avatar_color', accent);
  var width = Math.max(280, Math.min(900, parseInt(settings.width || '520', 10) || 520));
  var radius = Math.max(0, Math.min(40, parseInt(settings.radius || '12', 10) || 12));
  var fontMap = {{system:'Inter,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif',serif:'Georgia,"Times New Roman",serif',rounded:'ui-rounded,"SF Pro Rounded",Nunito,Inter,system-ui,sans-serif',condensed:'"Arial Narrow","Roboto Condensed",Arial,sans-serif'}};
  var shadowMap = {{none:'none',soft:'0 8px 22px rgba(15,23,42,.08)',medium:'0 12px 34px rgba(15,23,42,.12)',bold:'0 22px 60px rgba(15,23,42,.22)'}};
  var shadow = shadowMap[settings.shadow] || shadowMap.medium;
  var fontFamily = fontMap[settings.font] || fontMap.system;
  style.textContent = '.warren-reputation-widget{{--wrw-accent:' + accent + ';--wrw-bg:' + background + ';--wrw-head:' + headerBackground + ';--wrw-text:' + textColor + ';--wrw-muted:' + mutedColor + ';--wrw-star:' + starColor + ';--wrw-border:' + borderColor + ';--wrw-button-text:' + buttonText + ';--wrw-avatar:' + avatarColor + ';font-family:' + fontFamily + ';border:1px solid var(--wrw-border);border-radius:' + radius + 'px;background:var(--wrw-bg);color:var(--wrw-text);max-width:' + width + 'px;box-shadow:' + shadow + ';overflow:hidden}}.warren-reputation-widget *{{box-sizing:border-box}}.wrw-theme-dark{{background:var(--wrw-bg);color:var(--wrw-text);border-color:var(--wrw-border)}}.wrw-theme-brand .wrw-head{{background:var(--wrw-head)}}.wrw-layout-badge{{display:inline-flex;align-items:stretch;max-width:none}}.wrw-layout-badge .wrw-head{{padding:12px 14px}}.wrw-layout-badge .wrw-body{{display:none}}.wrw-layout-badge .wrw-actions{{padding:12px}}.wrw-layout-compact .wrw-body{{display:none}}.wrw-layout-popup{{display:none}}.wrw-head{{background:var(--wrw-head);color:#fff;padding:16px 18px}}.wrw-brand{{font-size:15px;font-weight:850;margin:0 0 6px}}.wrw-score{{display:flex;align-items:baseline;gap:8px;flex-wrap:wrap}}.wrw-rating{{font-size:30px;font-weight:900;line-height:1}}.wrw-stars{{color:var(--wrw-star);letter-spacing:1px;font-size:14px}}.wrw-count{{color:rgba(255,255,255,.78);font-size:13px}}.wrw-body{{padding:14px 18px;display:grid;gap:10px}}.wrw-review{{border-top:1px solid var(--wrw-border);padding-top:10px}}.wrw-review:first-child{{border-top:0;padding-top:0}}.wrw-reviews-grid .wrw-body{{grid-template-columns:repeat(auto-fit,minmax(160px,1fr))}}.wrw-reviews-grid .wrw-review{{border:1px solid var(--wrw-border);border-radius:' + Math.max(6, radius - 3) + 'px;padding:10px}}.wrw-reviews-scroller .wrw-body,.wrw-layout-carousel .wrw-body{{display:flex;overflow-x:auto;scroll-snap-type:x mandatory}}.wrw-reviews-scroller .wrw-review,.wrw-layout-carousel .wrw-review{{min-width:220px;scroll-snap-align:start;border:1px solid var(--wrw-border);border-radius:' + Math.max(6, radius - 3) + 'px;padding:10px}}.wrw-review-top{{display:flex;justify-content:space-between;gap:10px;font-size:12px;color:var(--wrw-muted);font-weight:750}}.wrw-text{{font-size:13px;line-height:1.4;color:var(--wrw-text);margin-top:5px}}.wrw-actions{{display:flex;gap:8px;flex-wrap:wrap;padding:0 18px 16px}}.wrw-btn{{display:inline-flex;align-items:center;justify-content:center;border-radius:' + Math.max(6, radius - 4) + 'px;padding:9px 12px;font-size:13px;font-weight:850;text-decoration:none}}.wrw-primary{{background:var(--wrw-accent);color:var(--wrw-button-text)}}.wrw-secondary{{border:1px solid var(--wrw-border);color:var(--wrw-text);background:var(--wrw-bg)}}.wrw-toast{{position:fixed;z-index:2147483000;width:min(360px,calc(100vw - 28px));border:1px solid var(--wrw-border);border-radius:' + Math.max(10, radius + 2) + 'px;background:var(--wrw-bg);color:var(--wrw-text);box-shadow:' + shadowMap.bold + ';padding:13px 14px;font-family:' + fontFamily + ';opacity:0;transform:translateY(12px);pointer-events:auto;transition:opacity .28s ease,transform .28s ease}}.wrw-toast.show{{opacity:1;transform:translateY(0)}}.wrw-toast.bottom-left{{left:18px;bottom:18px}}.wrw-toast.bottom-right{{right:18px;bottom:18px}}.wrw-toast.top-left{{left:18px;top:18px}}.wrw-toast.top-right{{right:18px;top:18px}}.wrw-toast-top{{display:flex;gap:10px;align-items:flex-start}}.wrw-avatar{{width:38px;height:38px;border-radius:50%;background:var(--wrw-avatar);color:var(--wrw-button-text);display:grid;place-items:center;font-weight:900;flex:0 0 auto}}.wrw-toast-title{{font-size:13px;font-weight:900;color:var(--wrw-text)}}.wrw-toast-stars{{font-size:12px;color:var(--wrw-star);letter-spacing:1px;margin-top:2px}}.wrw-toast-text{{font-size:12px;color:var(--wrw-muted);line-height:1.35;margin-top:5px}}.wrw-toast-foot{{display:flex;justify-content:space-between;align-items:center;gap:10px;margin-top:9px;font-size:11px;color:var(--wrw-muted)}}.wrw-toast a{{color:var(--wrw-accent);font-weight:850;text-decoration:none}}';
...
def _wp_security_challenge_error(status_code, response_text, action_label="Publish"):
    ...
    if "sgcaptcha" in body_lower or "/.well-known/sgcaptcha" in body_lower or (status_code == 202 and "captcha" in body_lower):
        return (
            f"{prefix}: the host returned a CAPTCHA challenge (HTTP 202) before WordPress could process "
            "the publish request. The request is being blocked at the hosting/firewall layer."
        )
    if status_code == 202:
        return (
...
def _wp_front_publish_url(brand):
    wp_url = (brand.get("wp_site_url") or "").strip().rstrip("/")
    return f"{wp_url}/?gromore_warren_publish=1"


def _wp_parse_helper_publish_response(resp, action_label):
    if resp.status_code not in (200, 201):
        return {
            "ok": False,
            "error": _wp_security_challenge_error(resp.status_code, resp.text, action_label),
            "missing_endpoint": resp.status_code == 404,
            "blocked_by_waf": _wp_should_try_custom_endpoint(resp.status_code, resp.text),
        }
    ...
    if not data and text in {"0", "-1"}:
        return {
            "ok": False,
            "error": "GroMore/Warren WordPress helper plugin is not installed or is older than version 1.2.0.",
    ...
    if not data:
        return {
            "ok": False,
            "error": "GroMore/Warren WordPress helper endpoint did not return JSON. Install or update the helper plugin.",
            "missing_endpoint": not _wp_should_try_custom_endpoint(resp.status_code, resp.text),
            "blocked_by_waf": _wp_should_try_custom_endpoint(resp.status_code, resp.text),
        }
    ...
    if data.get("ok") is False or data.get("success") is False:
        ...
        if isinstance(data.get("data"), dict):
            message = data["data"].get("message") or data["data"].get("error") or ""
        return {
            "ok": False,
            "error": str(data.get("error") or data.get("message") or message or f"{action_label} failed.")[:200],
    ...
    if not wp_id:
        return {"ok": False, "error": f"{action_label} did not return a post/page id.", "missing_endpoint": False, "blocked_by_waf": False}
    return {"ok": True, "wp_id": wp_id, "wp_url": wp_url}


def _wp_helper_payload_steps(payload):
    item_type = payload.get("type") or "post"
    final_status = (payload.get("status") or "publish").strip().lower()
    ...
    if item_id:
        ...
        update_payload["status"] = "draft" if final_status != "draft" else "draft"
        steps = [update_payload]
    else:
        create_payload = {
            "type": item_type,
    ...
    if final_status and final_status != "draft":
        steps.append({"type": item_type, "status": final_status})
    return steps


def _publish_via_warren_form_endpoint(brand, payload, endpoint_url, action_name="", helper_label="GroMore helper publish"):
    import json
    import requests as req_lib
    ...
    last_result = {}
    item_id = int(payload.get("id") or 0)
    for step_payload in _wp_helper_payload_steps(payload):
        if item_id and not step_payload.get("id"):
            step_payload["id"] = item_id
        form_data = {
            "payload": json.dumps(step_payload),
        ...
        try:
            ...
            return {"ok": False, "error": f"{helper_label} failed: {str(exc)[:160]}", "missing_endpoint": False, "blocked_by_waf": False}

        last_result = _wp_parse_helper_publish_response(resp, helper_label)
        if not last_result.get("ok"):
            return last_result
        item_id = int(last_result.get("wp_id") or item_id or 0)

    return last_result or {"ok": False, "error": f"{helper_label} did not run.", "missing_endpoint": False, "blocked_by_waf": False}


def _publish_via_warren_front_endpoint(brand, payload):
    """Fallback through the normal WordPress front controller, avoiding common blocked publish routes."""
    return _publish_via_warren_form_endpoint(
        brand,
        payload,
        _wp_front_publish_url(brand),
        helper_label="GroMore front publish",
    )


def _publish_via_warren_ajax_endpoint(brand, payload):
    """Fallback for sites where the front helper URL is unavailable but admin-ajax is allowed."""
    return _publish_via_warren_form_endpoint(
        brand,
        payload,
        _wp_ajax_publish_url(brand),
        action_name="gromore_warren_publish",
        helper_label="GroMore AJAX publish",
    )
...
def _publish_to_wp(brand, title, content, excerpt="", slug="",
    ...
    def _fallback_or_error(status_code, response_text):
        ...
        custom_result = _publish_via_warren_front_endpoint(brand, warren_payload)
...
_SCHEDULER_PLATFORM_ICONS = {
    "facebook": "bi-facebook",
    "instagram": "bi-instagram",
    "linkedin": "bi-linkedin",
    "nextdoor": "bi-house-heart",
    "google_business": "bi-google",
}


def _scheduler_platform_label(platform):
    return _SCHEDULER_PLATFORM_LABELS.get(str(platform or "").strip().lower(), str(platform or "Social").title())


def _scheduler_platform_icon(platform):
    return _SCHEDULER_PLATFORM_ICONS.get(str(platform or "").strip().lower(), "bi-share")


def _normalize_scheduler_platforms(value):
    if isinstance(value, str):
    ...
    for item in raw_items:
        platform = str(item or "").strip().lower().replace(" ", "_")
        if platform in {"fb", "facebook_page"}:
            platform = "facebook"
        if platform in {"ig", "instagram_business"}:
            platform = "instagram"
        if platform in {"gmb", "gbp", "google"}:
            platform = "google_business"
        if platform in _SCHEDULER_PLATFORM_LABELS and platform not in platforms:
            platforms.append(platform)
    return platforms or ["facebook"]


def _build_scheduler_channels(brand, connections):
    brand = brand or {}
    connections = connections or {}
    meta_conn = connections.get("meta") if (connections.get("meta") or {}).get("status") == "connected" else None
    google_conn = connections.get("google") if (connections.get("google") or {}).get("status") == "connected" else None
    page_id = str(brand.get("facebook_page_id") or "").strip()
    instagram_id = str(brand.get("instagram_account_id") or "").strip()
    linkedin_org_id = str(brand.get("linkedin_organization_id") or "").strip()
    nextdoor_url = str(brand.get("nextdoor_business_url") or "").strip()

    channels = [
        {
            "key": "facebook",
            "label": "Facebook Page",
            "icon": _scheduler_platform_icon("facebook"),
            "ready": bool(meta_conn and page_id),
            "direct": True,
            "default": bool(meta_conn and page_id),
            "detail": "Schedules directly through the connected Meta Page.",
            "setup": "" if (meta_conn and page_id) else "Connect Meta and save the main Facebook Page ID.",
        },
        {
            "key": "instagram",
            "label": "Instagram",
            "icon": _scheduler_platform_icon("instagram"),
            "ready": bool(meta_conn and (instagram_id or page_id)),
            "direct": True,
            "default": False,
            "detail": "WARREN queues Instagram image posts and publishes them through Meta when due. Requires a Professional account and content publishing permission.",
            "setup": "" if meta_conn and (instagram_id or page_id) else "Connect Meta and save an Instagram Professional Account ID or a Facebook Page linked to Instagram.",
        },
        {
            "key": "linkedin",
            "label": "LinkedIn",
            "icon": _scheduler_platform_icon("linkedin"),
            "ready": False,
            "direct": False,
            "default": False,
            "detail": "LinkedIn organic posting is supported by LinkedIn's API, but WARREN still needs LinkedIn OAuth and organization permission wiring.",
            "setup": "Roadmap: add LinkedIn OAuth with w_organization_social and save the organization ID.",
            "configured": bool(linkedin_org_id),
        },
        {
            "key": "nextdoor",
            "label": "Nextdoor",
            "icon": _scheduler_platform_icon("nextdoor"),
            "ready": False,
            "direct": False,
            "default": False,
            "detail": "Nextdoor business posts are useful, but public organic posting access is not the same as Meta scheduling.",
            "setup": "Save the business URL for tracking. Direct posting requires partner/API approval if available.",
            "configured": bool(nextdoor_url),
        },
        {
            "key": "google_business",
            "label": "Google Business Profile",
            "icon": _scheduler_platform_icon("google_business"),
            "ready": False,
            "direct": False,
            "default": False,
            "detail": "GBP updates are a good next channel once the Google Business Profile posting scope is approved.",
            "setup": "Roadmap: add GBP local post permissions and location selection.",
            "configured": bool(google_conn and brand.get("google_place_id")),
        },
    ]
    return channels
...
def _decorate_scheduler_posts(posts):
    ...
    for post in posts or []:
        ...
        item["platform_label"] = _scheduler_platform_label(item.get("platform"))
        item["platform_icon"] = _scheduler_platform_icon(item.get("platform"))
...
def _scheduler_post_payload(post):
    ...
    return {
        ...
        "platform_icon": item.get("platform_icon") or "",
...
def _scheduler_utc_string(dt):
    if not dt:
        return ""
    return dt.astimezone(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")


def _get_scheduler_meta_tokens(db, brand_id, brand):
    from webapp.api_bridge import _get_meta_token, _get_page_access_token

    connections = db.get_brand_connections(brand_id)
    meta_conn = connections.get("meta")
    if not meta_conn or meta_conn.get("status") != "connected":
        raise ValueError("Meta account not connected. Reconnect in Connections.")
    user_token = _get_meta_token(db, brand_id, meta_conn)
    if not user_token:
        raise ValueError("Meta access token expired. Reconnect in Connections.")
    page_id = str((brand or {}).get("facebook_page_id") or "").strip()
    page_token = ""
    if page_id:
        page_token = _get_page_access_token(page_id, user_token) or ""
    return {
...
def _resolve_instagram_account_id(brand, meta_context):
    instagram_id = str((brand or {}).get("instagram_account_id") or "").strip()
    if instagram_id:
        return instagram_id
    page_id = (meta_context or {}).get("page_id") or ""
    page_token = (meta_context or {}).get("page_token") or ""
    ...
    try:
        ...
        if resp.status_code != 200:
            return ""
        return str(((data or {}).get("instagram_business_account") or {}).get("id") or "").strip()
    except Exception:
        return ""


def _schedule_facebook_post_row(db, brand_id, brand, meta_context, *, message, first_comment, scheduled_at, sched_dt,
                                image_url="", link_url="", post_type=""):
    page_id = (meta_context or {}).get("page_id") or ""
    page_token = (meta_context or {}).get("page_token") or ""
    if not page_id:
        raise ValueError("No Facebook page connected.")
    if not page_token:
        raise ValueError("Could not get page access token. Check page permissions.")
    ...
    post_id = db.save_scheduled_post(
        brand_id,
        "facebook",
        message,
        scheduled_at,
        ...
        scheduled_at_utc=_scheduler_utc_string(sched_dt),
    )
    try:
        resp = req_lib.post(fb_url, data=payload, timeout=30)
        ...
        raise ValueError(f"Facebook API error: {exc}") from exc

    fb_post_id = resp_data.get("id") or resp_data.get("post_id") or ""
    if resp.status_code == 200 and fb_post_id:
        db.update_scheduled_post_status(post_id, "scheduled", fb_post_id=fb_post_id)
        return {"platform": "facebook", "post_id": post_id, "external_post_id": fb_post_id, "status": "scheduled"}

    error_msg = resp_data.get("error", {}).get("message", resp.text[:300])
    db.update_scheduled_post_status(post_id, "failed", error_message=error_msg)
    raise ValueError(f"Facebook rejected the post: {error_msg}")


def _queue_instagram_post_row(db, brand_id, brand, meta_context, *, message, first_comment, scheduled_at, sched_dt,
                              image_url="", link_url="", post_type=""):
    if not image_url:
        raise ValueError("Instagram posts need an image. Add an image URL or upload one from Drive.")
    instagram_id = _resolve_instagram_account_id(brand, meta_context)
    if not instagram_id:
        raise ValueError("No Instagram Professional Account found. Save the Instagram Account ID or connect it to the Facebook Page.")
    metadata = json.dumps({
    ...
    db.update_scheduled_post_status(post_id, "queued")
    return {"platform": "instagram", "post_id": post_id, "external_post_id": "", "status": "queued"}


def _schedule_social_post_for_platform(db, brand_id, brand, meta_context, platform, **kwargs):
    if platform == "facebook":
        return _schedule_facebook_post_row(db, brand_id, brand, meta_context, **kwargs)
    if platform == "instagram":
        return _queue_instagram_post_row(db, brand_id, brand, meta_context, **kwargs)
    raise ValueError(f"{_scheduler_platform_label(platform)} is not ready for direct scheduling yet.")
...
def client_post_scheduler():
    ...
    scheduler_channels = _build_scheduler_channels(brand, connections)
    has_scheduler_channel = any(ch.get("ready") and ch.get("direct") for ch in scheduler_channels)
    ...
    pending_count = sum(1 for p in posts if p.get("status") in ("pending", "queued", "scheduled"))
    ...
    return render_template(
        ...
        has_scheduler_channel=has_scheduler_channel,
        scheduler_channels=scheduler_channels,
        has_drive=has_drive,
...
def client_post_scheduler_posts():
    ...
    return jsonify(
        ...
        pending_count=sum(1 for post in posts if post.get("status") in ("pending", "queued", "scheduled")),
...
def client_schedule_post():
    """Schedule a single social post across ready publishing channels."""
    db = _get_db()
    brand_id = session["client_brand_id"]
    brand = db.get_brand(brand_id)
    if not brand:
    ...
    post_type = _normalize_facebook_post_type(data.get("post_type"))
    platforms = _normalize_scheduler_platforms(data.get("platforms") or data.get("platform") or ["facebook"])
    ...
    try:
        meta_context = _get_scheduler_meta_tokens(db, brand_id, brand)
    except ValueError as exc:
        return jsonify(ok=False, error=str(exc)), 400
    ...
    for platform in platforms:
        try:
            results.append(_schedule_social_post_for_platform(
                db,
                brand_id,
                brand,
                meta_context,
            ...
            current_app.logger.exception("social scheduler failed for brand %s platform %s", brand_id, platform)
            errors.append({"platform": platform, "error": str(exc)[:240]})

    if results:
        post_payloads = []
        for result in results:
            post = db.get_scheduled_post(result.get("post_id"))
            if post:
                post_payloads.append(_scheduler_post_payload(post))
        return jsonify(
            ok=True,
            ...
            errors=errors,
            results=results,
            post_id=results[0].get("post_id"),
            fb_post_id=results[0].get("external_post_id") if results[0].get("platform") == "facebook" else "",
            post_type=post_type,
            posts=post_payloads,
            post=post_payloads[0] if post_payloads else None,
        )

    error_text = "; ".join(f"{_scheduler_platform_label(e['platform'])}: {e['error']}" for e in errors) or "No channels could be scheduled."
    return jsonify(ok=False, error=error_text, errors=errors), 400
...
def client_schedule_posts_bulk():
    ...
    for post in posts:
        ...
        platforms = _normalize_scheduler_platforms(post.get("platforms") or post.get("platform") or data.get("platforms") or ["facebook"])
        ...
        for platform in platforms:
            try:
                result = _schedule_social_post_for_platform(
                    db,
                    brand_id,
                    brand,
                    meta_context,
    ...
    result = {"ok": True, "scheduled": scheduled, "queued": queued, "errors": errors, "total": len(posts), "error_details": error_details[:20]}
    if errors and not (scheduled or queued):
        result["ok"] = False
        result["error"] = f"All {errors} channel posts failed. Check dates, content, and platform requirements."
    return jsonify(result)
